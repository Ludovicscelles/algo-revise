...
describe.each([
  ...
  it("should throw an error for non-integer inputs", () => {
    expect(() => fn(2.5)).toThrowError("La saisie doit être un nombre entier.");
...
describe("Tables class", () => {
  it("should return formated multiplication tables up to the given number", () => {
    const tables = new Tables(3);
    const result = tables.generate();
    const expectedOutput =
      "Table de 1\n" +
      "-----------------\n" +
      "1 x 1 = 1\n" +
  ...
  it("should throw an error for non-integer inputs", () => {
    expect(() => new Tables(2.5)).toThrowError(
      "La saisie doit être un nombre entier."
    );
    expect(() => new Tables("string" as unknown as number)).toThrowError(
      "La saisie doit être un nombre entier."
    );
    expect(() => new Tables(null as unknown as number)).toThrowError(
      "La saisie doit être un nombre entier."
    );
    expect(() => new Tables(undefined as unknown as number)).toThrowError(
      "La saisie doit être un nombre entier."
    );
    expect(() => new Tables({} as unknown as number)).toThrowError(
      "La saisie doit être un nombre entier."
    );
  });
});
